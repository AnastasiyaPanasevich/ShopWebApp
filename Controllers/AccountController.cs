...
namespace ShopWebApp
{
    public class AccountController : Controller
    {
        ...
        {
            var model = new AccountModel("My Account");
        ...
        public async Task<IActionResult> TryEdit(AccountModel input)
        {
            ...
            if (needRelog)
            {
                ...
                if (changedPass && changedEmail)
                    input.Message = "Successfully changed email and password";
                else if (changedEmail)
                    input.Message = "Successfully changed email";
                else
                    input.Message = "Successfully changed password";
                return View(input);
            }
        ...
        public IActionResult Register(AccountModel input)
        {
            if (User.Identity.IsAuthenticated)
                return Redirect(Url.Action("index", "home"));
            var model = new AccountModel("Register");
            return View(model);
        }
